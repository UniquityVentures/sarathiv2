...
class Assessment(models.Model):
    ...
    _sql_constraints = [
        ...
        ('max_marks_positive', 'CHECK(max_marks > 0)', 'Maximum Marks must be positive!'),
        ('passing_marks_valid', 'CHECK(passing_marks >= 0 AND passing_marks <= max_marks)', 'Passing Marks must be between 0 and Maximum Marks!')
    ]
...
class StudentAssessment(models.Model):
    ...
    status = fields.Selection([('PASS', 'Pass'), ('FAIL', 'Fail')], string='Status', compute='_compute_status', store=True)
    ...
    def _check_marks_valid(self):
        for record in self:
            if record.marks is not False and record.assessment_id and record.marks > record.assessment_id.max_marks:
